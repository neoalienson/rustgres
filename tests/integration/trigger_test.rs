...
use rustgres::parser::ast::{TriggerTiming, TriggerEvent};
...
fn test_multiple_triggers() {
    ...
    let mut parser1 = Parser::new("CREATE TRIGGER t1 BEFORE INSERT ON users FOR EACH ROW BEGIN END").unwrap();
    let stmt1 = parser1.parse().unwrap();
    
    let mut parser2 = Parser::new("CREATE TRIGGER t2 AFTER UPDATE ON orders FOR EACH ROW BEGIN END").unwrap();
...
fn test_trigger_timing_preserved() {
    ...
    let mut parser = Parser::new("CREATE TRIGGER t AFTER DELETE ON users FOR EACH ROW BEGIN END").unwrap();
    let stmt = parser.parse().unwrap();
