        ...
        match plan {
            LogicalPlan::Filter { input, predicate } => match *input {
                LogicalPlan::Scan { table, filter: _, columns } => {
                    LogicalPlan::Scan { table, filter: Some(predicate), columns }
                }
                ...
                other => LogicalPlan::Filter { input: Box::new(self.apply(other)), predicate },
            },
            LogicalPlan::Project { input, columns } => {
                LogicalPlan::Project { input: Box::new(self.apply(*input)), columns }
            }
            LogicalPlan::Join { left, right, condition } => LogicalPlan::Join {
