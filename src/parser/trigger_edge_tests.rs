...
mod tests {
    ...
    use crate::parser::ast::{TriggerTiming, TriggerEvent};

    #[test]
    fn test_trigger_all_timing_combinations() {
        let cases = vec![
            ("CREATE TRIGGER t BEFORE INSERT ON t FOR EACH ROW BEGIN END", TriggerTiming::Before, TriggerEvent::Insert),
            ("CREATE TRIGGER t BEFORE UPDATE ON t FOR EACH ROW BEGIN END", TriggerTiming::Before, TriggerEvent::Update),
            ("CREATE TRIGGER t BEFORE DELETE ON t FOR EACH ROW BEGIN END", TriggerTiming::Before, TriggerEvent::Delete),
            ("CREATE TRIGGER t AFTER INSERT ON t FOR EACH ROW BEGIN END", TriggerTiming::After, TriggerEvent::Insert),
            ("CREATE TRIGGER t AFTER UPDATE ON t FOR EACH ROW BEGIN END", TriggerTiming::After, TriggerEvent::Update),
            ("CREATE TRIGGER t AFTER DELETE ON t FOR EACH ROW BEGIN END", TriggerTiming::After, TriggerEvent::Delete),
        ];
    ...
    fn test_trigger_when_with_comparison() {
        let mut parser = Parser::new("CREATE TRIGGER t BEFORE INSERT ON users FOR EACH ROW WHEN (age > 18) BEGIN END").unwrap();
        let stmt = parser.parse().unwrap();
    ...
    fn test_trigger_case_insensitive() {
        let mut parser = Parser::new("create trigger MyTrigger before insert on MyTable for each row begin end").unwrap();
        let stmt = parser.parse().unwrap();
    ...
    fn test_trigger_empty_body() {
        let mut parser = Parser::new("CREATE TRIGGER t BEFORE INSERT ON users FOR EACH ROW BEGIN END").unwrap();
        let stmt = parser.parse().unwrap();
    ...
    fn test_trigger_all_comparison_operators() {
        ...
        for op in ops {
            let sql = format!("CREATE TRIGGER t BEFORE INSERT ON users FOR EACH ROW WHEN (x {} 10) BEGIN END", op);
            let mut parser = Parser::new(&sql).unwrap();
            let stmt = parser.parse().unwrap();
